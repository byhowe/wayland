...
{
    // LIFETIME BOUND ANALYSIS: Why `type Output<'buf>: 'buf` was removed
    //
    ...
    //
    // ORIGINAL REASONING:
    // The `'buf` bound was added thinking that any type returned by a read function
    // should live at least as long as `'buf` (the buffer's lifetime). The logic
    // was: "if the buffer dies, any data borrowed from it should also become
    // invalid."
    //
    // WHY IT WORKED INITIALLY:
    // For simple cases with only `Cow<'buf, str>` and `Cow<'buf, [u8]>`, this bound
    // was redundant but harmless:
    // - `Cow<'buf, str>` already explicitly uses `'buf`
    // - The compiler automatically ensures `'buf` outlives itself
    ...
    //    - `'buf`: How long the message buffer memory lives
    //    - `'fd`: How long the file descriptor handle remains valid
    //
    // THE ACTUAL CONSTRAINT PROBLEM:
    // With `Output<'buf>: 'buf` and `Fd = BorrowedFd<'fd>`, the compiler required:
    // `BorrowedFd<'fd>: 'buf` which means `'fd: 'buf`
    //
    ...
    // 1. For `Cow<'buf, T>` types, the `'buf` constraint is already implicit
    ...
    //
    // NOTE: The `Wire` trait still has `Output<'buf>: 'buf` because it deals with
